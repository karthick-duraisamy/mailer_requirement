...
function App() {
  ...
  const [aiReplyStates, setAiReplyStates] = useState<Map<string, AiReplyState>>(
    new Map(),
  );
  ...
  const emailCounts = useMemo(() => {
    ...
    counts.starred = emails.filter(
      (email) => email.isStarred && !email.isRead,
    ).length;
  ...
  const conversations = useMemo(() => {
    ...
    return Array.from(conversationMap.values())
      .map((conversationEmails) => {
        ...
        const allMessages = conversationEmails
          .flatMap((email) => email.messages)
          .sort(
            (a, b) =>
              new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime(),
          );
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (email) =>
          ...
          (email.customLabels &&
            email.customLabels.some((labelId) => {
      ...
      case "star":
        setEmails((prevEmails) =>
          prevEmails.map((email) => {
            ...
            return prevState
              ? { ...email, isStarred: prevState.isStarred }
              : email;
          }),
        );
        break;
      ...
      case "labelChange":
        setEmails((prevEmails) =>
          prevEmails.map((email) => {
            ...
            return prevState
              ? { ...email, customLabels: prevState.customLabels }
              : email;
          }),
        );
        break;
  ...
  const getContextualResponse = (email: Email) => {
    const content =
      email.messages[email.messages.length - 1].content.toLowerCase();
          ...
          {isFullPageView ? (
            <ConversationThread
              ...
              onAiReplyStateChange={(newState) =>
                selectedEmail?.id &&
                updateAiReplyState(selectedEmail.id, newState)
              }
              customLabels={customLabels}
