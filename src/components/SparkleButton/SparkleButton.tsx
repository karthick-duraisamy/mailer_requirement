...
const SparkleButton: React.FC<SparkleButtonProps> = ({ text, onClick, fontSize, borderRad }) => {
  return (
    <button className="sparkle-button" disabled={text === ''} onClick={onClick} style={{fontSize: fontSize+"px", borderRadius: borderRad+"px"}}>
      <SparkleSVG />
      <span className="button-text">{text === '' ? <LoadingIndicator /> : text}</span>
    </button>
  );
};
