...
const SignatureSetup: React.FC<SignatureSetupProps> = ({ isOpen, onClose }) => {
  ...
  const handleSetDefault = (id: number) => {
    setSignatures(signatures.map(sig => ({ ...sig, isDefault: sig.id === id })));
  };
          ...
          {(isCreatingNew || editingSignature) ? (
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium text-gray-900">Your Signatures</h3>
                <button
              ...
              <div className="space-y-3">
                {signatures.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm">Click "Create New" to add your first signature</p>
                  </div>
                ) : (
                  signatures.map((signature) => (
                    <div key={signature.id} className="border border-gray-200 rounded-lg p-4">
                      ...
                      <div className="bg-gray-50 rounded p-3 text-sm text-gray-700">
                        <div className="whitespace-pre-wrap">{signature.content}</div>
                      </div>
                    </div>
                  ))
                )}
